import { Button, HStack, Image, Navigation, ScrollView, Spacer, Text, TextField, VStack, ZStack, useState } from 'scripting'
...
export const SubmitForm = ({ onSuccess, themeMode }: SubmitFormProps) => {
  ...
  const labelSecondaryColor = themedColors.labelSecondary(themeMode)
      ...
      <ScrollView scrollDismissesKeyboard="interactively">
        <VStack padding={16} spacing={16}>
          <VStack padding={20} background={colors.cardBackground} clipShape={{ type: 'rect', cornerRadius: 12 }} spacing={16}>
            <Text font={14} foregroundStyle={labelSecondaryColor}>插件图标</Text>
            ...
            {iconMode === 'svg' ? (
              ...
              <VStack alignment="leading" spacing={8}>
                ...
                <ZStack alignment="leading">
                  {!symbol && <Text font={14} foregroundStyle={placeholderColor} padding={{ leading: 4 }}>请输入 SF Symbol 名称，如 star.fill</Text>}
                  <TextField title="" value={symbol} onChanged={setSymbol} textInputAutocapitalization="never" autocorrectionDisabled foregroundStyle={colors.textPrimary} />
                </ZStack>
              </VStack>
            )}
          </VStack>
          ...
          <VStack padding={20} background={colors.cardBackground} clipShape={{ type: 'rect', cornerRadius: 12 }} spacing={16}>
            <Text font={14} foregroundStyle={labelSecondaryColor}>基本信息</Text>

            <VStack alignment="leading" spacing={8}>
              <Text font={14} fontWeight="medium" foregroundStyle={labelColor}>插件名称 *</Text>
              <ZStack alignment="leading">
                {!name && <Text font={14} foregroundStyle={placeholderColor} padding={{ leading: 4 }}>请输入插件名称</Text>}
                <TextField title="" value={name} onChanged={setName} foregroundStyle={colors.textPrimary} />
              </ZStack>
            </VStack>

            <VStack alignment="leading" spacing={8}>
              <Text font={14} fontWeight="medium" foregroundStyle={labelColor}>插件描述 *</Text>
              <ZStack alignment="leading">
                {!description && <Text font={14} foregroundStyle={placeholderColor} padding={{ leading: 4 }}>请输入插件描述</Text>}
                <TextField title="" value={description} onChanged={setDescription} foregroundStyle={colors.textPrimary} />
              </ZStack>
            </VStack>

            <VStack alignment="leading" spacing={8}>
              <Text font={14} fontWeight="medium" foregroundStyle={labelColor}>作者</Text>
              <ZStack alignment="leading">
                {!author && <Text font={14} foregroundStyle={placeholderColor} padding={{ leading: 4 }}>脚本作者</Text>}
                <TextField title="" value={author} onChanged={setAuthor} foregroundStyle={colors.textPrimary} />
              </ZStack>
            </VStack>

            <VStack alignment="leading" spacing={8}>
              <Text font={14} fontWeight="medium" foregroundStyle={labelColor}>插件链接 *</Text>
              <ZStack alignment="leading">
                {!url && <Text font={14} foregroundStyle={placeholderColor} padding={{ leading: 4 }}>请输入插件下载链接</Text>}
          ...
          <VStack padding={16} background={themedColors.tipBackground(themeMode)} clipShape={{ type: 'rect', cornerRadius: 12 }} alignment="leading" spacing={8}>
